...
먼저, 편의상 입력으로 들어오는 문자열에는 영문 소문자 (a, b, ..., z)로만 구성되어 있다고 가정하자. 
영어에는 총 26개의 알파벳이 존재하므로 a에는 1, b에는 2, c에는 3, ..., z에는 26으로 고유한 번호를 부여할 수 있다. 
결과적으로 우리는 하나의 문자열을 수열로 변환할 수 있다. 예를 들어서 문자열 "abba"는 수열 1, 2, 2, 1로 나타낼 수 있다. 

해시 값을 계산하기 위해서 우리는 문자열 혹은 수열을 하나의 정수로 치환하려고 한다. 
간단하게는 수열의 값을 모두 더할 수도 있다. 해시 함수의 정의에서 유한한 범위의 출력을 가져야한다고 했으니까 적당히 큰 수 M으로 나눠주자. 
...
머리를 굴리면 수열의 각 항마다 고유한 계수를 부여하면 된다는 아이디어를 생각해 볼 수 있다. 
...
그리고 이 함수는 간단해보여도 자주 쓰이니까 기억해뒀다가 잘 써먹도록 하자. 
'''
...
for i in range(L):
    _sum += (ord(text[i]) - 96) * r ** i

print(_sum % M)

'''
회고 / TIL
- 문제가 꽤 길지만 요구하는 바는 간단했다. 
- 알파벳의 ord() 결과 숫자는 이제 기억할 법도 한데 매번 기억이 안 난다. 
- 서브태스크 문제는 처음 보는 것 같은데 (백린이라서...) 모든 테스트 케이스에 대해 맞춰야 100점이라고 한다. 
'''